...
static int rtp_h265_pack_nalu(struct rtp_encode_h265_t *packer, const uint8_t* nalu, int bytes, int mark)
{
	...
	// Marker bit should indicate end of an access unit (frame). Some encoders may output
	// trailing non-VCL NALUs (e.g., SEI/AUD) after VCL, which would otherwise cause
	// no marker for the whole frame and players may stall. Use mark unconditionally.
