...
inline std::string generate_function_signature(const char* type_caster_name_field,
                                               ...
                                               const std::type_info* const* types, size_t& type_index,
                                               size_t& arg_index) {
...
template <typename T> inline std::string generate_type_signature() {
    ...
    size_t type_index = 0;
    size_t arg_index = 0;
    return generate_function_signature(caster_name_field.text, &func_rec, descr_types.data(), type_index,
                                       arg_index);
}
...
class cpp_function : public function {
  ...
  protected:
    ...
    void initialize(Func&& f, Return (*)(Args...), const Extra&... extra) {
        ...
        static_assert(
            expected_num_args<Extra...>(sizeof...(Args), cast_in::args_pos >= 0, cast_in::has_kwargs),
        ...
        rec->impl = [](function_call& call) -> handle {
            ...
            if (call.func.is_setter) {
                ...
                result = cast_out::cast(std::move(args_converter).template call<Return, Guard>(cap->f),
                                        policy, call.parent);
            }
    ...
    void initialize_generic(unique_function_record&& unique_rec, const char* text,
        ...
        rec->is_constructor =
            (std::strcmp(rec->name, "__init__") == 0) || (std::strcmp(rec->name, "__setstate__") == 0);
        ...
        for (auto* it = chain_start; it != nullptr; it = it->next) {
            if (options::show_function_signatures() && std::strcmp(rec->name, "_pybind11_conduit_v1_") != 0) {
                if (index > 0) {
                    signatures += '\n';
                }
                if (chain) {
...
class multiple_interpreters {
  public:
    ...
    static multiple_interpreters not_supported() { return multiple_interpreters(level::not_supported); }
...
inline bool gil_not_used_option() {
    return false;
}
template <typename F, typename... O> bool gil_not_used_option(F&&, O&&... o);
template <typename... O> inline bool gil_not_used_option(mod_gil_not_used f, O&&... o) {
    return f.flag() || gil_not_used_option(o...);
}
template <typename F, typename... O> inline bool gil_not_used_option(F&&, O&&... o) {
...
template <typename... O> inline void* multi_interp_slot(multiple_interpreters mi, O&&... o) {
    switch (mi.value()) {
...
template <typename F, typename... O> inline void* multi_interp_slot(F&&, O&&... o) {
    return multi_interp_slot(o...);
}
#endif
...
class module_ : public object {
  public:
    ...
    \endrst */
    static module_ create_extension_module(const char* name, const char* doc, PyModuleDef* def,
...
template <> struct handle_type_name<module_> { static constexpr auto name = const_name("types.ModuleType"); };
...
class generic_type : public object {
  ...
  protected:
    ...
    void def_property_static_impl(const char* name, handle fget, handle fset,
        ...
        auto property =
            handle((PyObject*)(is_static ? get_internals().static_property_type : &PyProperty_Type));
...
template <typename /*Derived*/, typename F> auto method_adaptor(F&& f) -> decltype(std::forward<F>(f)) {
...
template <typename T, typename D> struct property_cpp_function_classic {
...
template <typename T, typename D> struct property_cpp_function_sh_raw_ptr_member {
...
template <typename T, typename D> struct property_cpp_function_sh_member_held_by_value {
    template <typename PM, must_be_member_function_pointer<PM> = 0>
    static cpp_function readonly(PM pm, const handle& hdl) {
        type_info* tinfo = get_type_info(typeid(T), /*throw_if_missing=*/true);
        if (tinfo->holder_enum_v == holder_enum_t::smart_holder) {
...
template <typename T, typename D> struct property_cpp_function_sh_unique_ptr_member {
    template <typename PM, must_be_member_function_pointer<PM> = 0>
...
struct property_cpp_function<T, D,
                             detail::enable_if_t<detail::all_of<
                                 std::is_pointer<D>, detail::both_t_and_d_use_type_caster_base<T, D>>::value>>
...
struct property_cpp_function<
    T, D,
    detail::enable_if_t<detail::all_of<
        detail::none_of<std::is_pointer<D>, std::is_array<D>, detail::is_instantiation<std::unique_ptr, D>,
...
struct property_cpp_function<
    T, D,
    detail::enable_if_t<
        detail::all_of<detail::is_instantiation<std::unique_ptr, D>,
...
template <typename> using default_holder_type = smart_holder;
#else
template <typename T> using default_holder_type = std::unique_ptr<T>;
#endif
...
template <typename type_, typename... options> class class_ : public detail::generic_type {
  ...
  public:
    ...
    class_& def_readonly(const char* name, const D C::*pm, const Extra&... extra) {
        ...
        def_property_readonly(name, property_cpp_function<type, D>::readonly(pm, *this),
                              return_value_policy::reference_internal, extra...);
        return *this;
    }
    ...
    class_& def_property_readonly_static(const char* name, const Getter& fget, const Extra&... extra) {
        return def_property_readonly_static(name, cpp_function(fget), return_value_policy::reference,
                                            extra...);
    }
  ...
  private:
    ...
    static void init_holder_from_existing(const detail::value_and_holder& v_h, const holder_type* holder_ptr,
    ...
    static bool
    try_initialization_using_shared_from_this(holder_type* uninitialized_location, WrappedType* value_ptr_w_t,
                                              const std::enable_shared_from_this<SomeBaseOfWrappedType>*) {
        auto shd_ptr =
            std::dynamic_pointer_cast<WrappedType>(detail::try_get_shared_from_this(value_ptr_w_t));
        if (!shd_ptr) {
            return false;
        }
    ...
    static void init_instance(detail::instance* inst, const void* holder_const_void_ptr) {
        ...
        } else if (!try_initialization_using_shared_from_this(uninitialized_location, value_ptr_w_t,
                                                              value_ptr_w_t)) {
            if (inst->owned) {
                ...
                new (uninitialized_location) holder_type(holder_type::from_raw_ptr_unowned(value_ptr_w_t));
            }
        }
        v_h.set_holder_constructed();
    }
...
template <typename type_, typename... options> using classh = class_<type_, smart_holder, options...>;
    ...
    PYBIND11_NOINLINE void init(bool is_arithmetic, bool is_convertible) {
        ...
        m_base.attr("__repr__") = cpp_function(
            [](const object& arg) -> str {
                ...
                return pybind11::str("<{}.{}: {}>").format(std::move(type_name), enum_name(arg), int_(arg));
            },
                ...
                cpp_function(
                    [](handle arg) -> std::string {
                        ...
                        if (((PyTypeObject*)arg.ptr())->tp_doc) {
                            docstring += std::string(reinterpret_cast<PyTypeObject*>(arg.ptr())->tp_doc);
                            docstring += "\n\n";
                        }
                        docstring += "Members:";
                        for (auto kv : entries) {
    ...
    m_base.attr(op) = cpp_function(                                                                          \
        ...
        if (is_convertible) {
            ...
            if (is_arithmetic) {
                ...
                m_base.attr("__invert__") = cpp_function([](const object& arg) { return ~(int_(arg)); },
...
#undef PYBIND11_ENUM_OP_STRICT
        ...
        m_base.attr("__hash__") = cpp_function([](const object& arg) { return int_(arg); }, name("__hash__"),
                                               is_method(m_base), pos_only());
    }
...
template <typename Type> class enum_ : public class_<Type> {
  public:
    ...
    enum_(const handle& scope, const char* name, const Extra&... extra)
        ...
        {
            if (detail::global_internals_native_enum_type_map_contains(std::type_index(typeid(Type)))) {
 ...
 */
template <typename Access, return_value_policy Policy, typename Iterator, typename Sentinel,
...
typing::Iterator<ValueType> make_iterator(Iterator first, Sentinel last, Extra&&... extra) {
    return detail::make_iterator_impl<detail::iterator_access<Iterator>, Policy, Iterator, Sentinel,
                                      ValueType, Extra...>(
...
template <return_value_policy Policy = return_value_policy::reference_internal, typename Iterator,
          typename Sentinel, typename KeyType = typename detail::iterator_key_access<Iterator>::result_type,
          typename... Extra>
typing::Iterator<KeyType> make_key_iterator(Iterator first, Sentinel last, Extra&&... extra) {
    return detail::make_iterator_impl<detail::iterator_key_access<Iterator>, Policy, Iterator, Sentinel,
                                      KeyType, Extra...>(
        std::forward<Iterator>(first), std::forward<Sentinel>(last), std::forward<Extra>(extra)...);
}
...
typing::Iterator<ValueType> make_value_iterator(Iterator first, Sentinel last, Extra&&... extra) {
    return detail::make_iterator_impl<detail::iterator_value_access<Iterator>, Policy, Iterator, Sentinel,
                                      ValueType, Extra...>(
        std::forward<Iterator>(first), std::forward<Sentinel>(last), std::forward<Extra>(extra)...);
}
...
template <return_value_policy Policy = return_value_policy::reference_internal, typename Type,
          typename ValueType =
              typename detail::iterator_access<decltype(std::begin(std::declval<Type&>()))>::result_type,
          typename... Extra>
typing::Iterator<ValueType> make_iterator(Type& value, Extra&&... extra) {
    return make_iterator<Policy>(std::begin(value), std::end(value), std::forward<Extra>(extra)...);
}
...
template <return_value_policy Policy = return_value_policy::reference_internal, typename Type,
          typename KeyType =
              typename detail::iterator_key_access<decltype(std::begin(std::declval<Type&>()))>::result_type,
          typename... Extra>
typing::Iterator<KeyType> make_key_iterator(Type& value, Extra&&... extra) {
    return make_key_iterator<Policy>(std::begin(value), std::end(value), std::forward<Extra>(extra)...);
}
...
typing::Iterator<ValueType> make_value_iterator(Type& value, Extra&&... extra) {
    return make_value_iterator<Policy>(std::begin(value), std::end(value), std::forward<Extra>(extra)...);
}
...
exception<CppException>& register_exception_impl(handle scope, const char* name, handle base, bool isLocal) {
    ...
    exc_storage.call_once_and_store_result([&]() { return exception<CppException>(scope, name, base); });
...
inline function get_type_override(const void* this_ptr, const type_info* this_type, const char* name) {
    ...
    if (override.is_cpp_function()) {
        with_internals(
            [&](internals& internals) { internals.inactive_override_cache.insert(std::move(key)); });
        return function();
    }
