...
static_assert(EIGEN_VERSION_AT_LEAST(3, 2, 7), "Eigen matrix support in pybind11 requires Eigen >= 3.2.7");
...
template <typename props, typename Type> handle eigen_ref_array(Type& src, handle parent = none()) {
...
template <typename Type> struct type_caster<Type, enable_if_t<is_eigen_dense_plain<Type>::value>> {
    using Scalar = typename Type::Scalar;
    static_assert(!std::is_pointer<Scalar>::value, PYBIND11_EIGEN_MESSAGE_POINTER_TYPES_ARE_NOT_SUPPORTED);
  ...
  private:
    // Cast implementation
    template <typename CType> static handle cast_impl(CType* src, return_value_policy policy, handle parent) {
        switch (policy) {
...
template <typename Type> struct type_caster<Type, enable_if_t<is_eigen_sparse<Type>::value>> {
    ...
    bool load(handle src, bool) {
        ...
        value = EigenMapSparseMatrix<Scalar, Type::Flags&(Eigen::RowMajor | Eigen::ColMajor), StorageIndex>(
            shape[0].cast<Index>(), shape[1].cast<Index>(), std::move(nnz), outerIndices.mutable_data(),
    ...
    static handle cast(const Type& src, return_value_policy /* policy */, handle /* parent */) {
        ...
        return matrix_type(
                   pybind11::make_tuple(std::move(data), std::move(innerIndices), std::move(outerIndices)),
                   pybind11::make_tuple(src.rows(), src.cols()))
            .release();
    }

    PYBIND11_TYPE_CASTER(Type, const_name<(Type::IsRowMajor) != 0>("scipy.sparse.csr_matrix[",
