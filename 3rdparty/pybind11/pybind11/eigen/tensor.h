...
static_assert(EIGEN_VERSION_AT_LEAST(3, 3, 0), "Eigen Tensor support in pybind11 requires Eigen >= 3.3.0");
...
template <typename Type, bool ShowDetails, bool NeedsWriteable = false> struct get_tensor_descriptor {
    static constexpr auto details =
        const_name<NeedsWriteable>(", \"flags.writeable\"", "") +
        const_name<static_cast<int>(Type::Layout) == static_cast<int>(Eigen::RowMajor)>(
            ", \"flags.c_contiguous\"", ", \"flags.f_contiguous\"");
    static constexpr auto value =
        const_name("typing.Annotated[") + io_name("numpy.typing.ArrayLike, ", "numpy.typing.NDArray[") +
        npy_format_descriptor<typename Type::Scalar>::name + io_name("", "]") + const_name(", \"[") +
...
template <typename T, int size> std::vector<T> convert_dsizes_to_vector(const Eigen::DSizes<T, size>& arr) {
...
template <typename T, int size> Eigen::DSizes<T, size> get_shape_for_array(const array& arr) {
...
template <typename Type> struct type_caster<Type, typename eigen_tensor_helper<Type>::ValidType> {
    ...
    template <typename C> static handle cast_impl(C* src, return_value_policy policy, handle parent) {
...
struct type_caster<Eigen::TensorMap<Type, Options>,
    ...
    bool load(handle src, bool /*convert*/) {
        ...
        auto result =
            get_array_data_for_type<typename get_storage_pointer_type<MapType>::SPT, needs_writeable>(arr);
    ...
    static constexpr bool needs_writeable = !std::is_const<
        typename std::remove_pointer<typename get_storage_pointer_type<MapType>::SPT>::type>::value;
#else
    // Handle Eigen bug
