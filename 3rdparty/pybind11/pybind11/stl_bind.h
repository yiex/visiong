...
void vector_if_equal_operator(enable_if_t<is_comparable<Vector>::value, Class_>& cl) {
    ...
    cl.def(
        "count", [](const Vector& v, const T& x) { return std::count(v.begin(), v.end(), x); }, arg("x"),
    ...
    cl.def(
        "__contains__",
        [](const Vector& v, const T& x) { return std::find(v.begin(), v.end(), x) != v.end(); }, arg("x"),
...
void vector_modifiers(enable_if_t<is_copy_constructible<typename Vector::value_type>::value, Class_>& cl) {
    ...
    cl.def(
        "extend", [](Vector& v, const Vector& src) { v.insert(v.end(), src.begin(), src.end()); }, arg("L"),
...
void vector_accessor(enable_if_t<!vector_needs_copy<Vector>::value, Class_>& cl) {
    ...
    cl.def(
        "__iter__",
        [](Vector& v) {
            return make_iterator<return_value_policy::reference_internal, ItType, ItType, T&>(v.begin(),
                                                                                              v.end());
        },
...
template <typename KeyType> str format_message_key_error(const KeyType& key) {
    object py_key;
    try {
        py_key = cast(key);
    } catch (const std::exception&) {
...
class_<Map, holder_type> bind_map(handle scope, const std::string& name, Args&&... args) {
    ...
    cl.def(
        "keys", [](Map& m) { return std::unique_ptr<KeysView>(new detail::KeysViewImpl<Map>(m)); },
        keep_alive<0, 1>() /* Essential: keep map alive while view exists */
    );

    cl.def(
        "values", [](Map& m) { return std::unique_ptr<ValuesView>(new detail::ValuesViewImpl<Map>(m)); },
        keep_alive<0, 1>() /* Essential: keep map alive while view exists */
    );

    cl.def(
        "items", [](Map& m) { return std::unique_ptr<ItemsView>(new detail::ItemsViewImpl<Map>(m)); },
        keep_alive<0, 1>() /* Essential: keep map alive while view exists */
    );
