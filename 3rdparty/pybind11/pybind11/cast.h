...
cast_op(make_caster<T>&& caster) {
    using result_t =
        typename make_caster<T>::template cast_op_type<typename std::add_rvalue_reference<T>::type>; // See PR
                                                                                                     // #4893
...
template <typename EnumType> class type_caster_enum_type {
  ...
  public:
    ...
    template <typename SrcType> static handle cast(SrcType&& src, return_value_policy, handle parent) {
        handle native_enum =
...
template <typename EnumType, typename SFINAE = void> struct type_caster_enum_type_enabled : std::true_type {};

template <typename T> struct type_uses_type_caster_enum_type {
...
template <typename T> bool isinstance_native_enum(handle obj, const std::type_info& tp) {
...
#define PYBIND11_TYPE_CASTER(type, py_name)                                                                  \
  ...
  public:                                                                                                    \
    ...
    template <typename T_, ::pybind11::detail::enable_if_t<                                                  \
...
struct type_caster<T, enable_if_t<std::is_arithmetic<T>::value && !is_std_char_type<T>::value>> {
  ...
  public:
        ...
        if (py_err || (std::is_integral<T>::value && sizeof(py_type) != sizeof(T) &&
            ...
            if (py_err && convert && (PyNumber_Check(src.ptr()) != 0)) {
                auto tmp = reinterpret_steal<object>(
                    std::is_floating_point<T>::value ? PyNumber_Float(src.ptr()) : PyNumber_Long(src.ptr()));
                PyErr_Clear();
    ...
    PYBIND11_TYPE_CASTER(T, io_name<std::is_integral<T>::value>("typing.SupportsInt", "int",
...
template <typename T> struct void_caster {
  public:
    ...
    static handle cast(T, return_value_policy /* policy */, handle /* parent */) { return none().release(); }
...
template <> class type_caster<bool> {
  ...
  private:
    ...
    static inline bool is_numpy_bool(handle object) {
        ...
        return std::strcmp("numpy.bool", type_name) == 0 || std::strcmp("numpy.bool_", type_name) == 0;
    }
};
...
template <typename StringType, bool IsView = false> struct string_caster {
  ...
  private:
    ...
    template <typename C = CharT> bool load_raw(enable_if_t<std::is_same<C, char>::value, handle> src) {
    ...
    template <typename C = CharT> bool load_raw(enable_if_t<!std::is_same<C, char>::value, handle>) {
        return false;
    }
};
        ...
        if (StringCaster::UTF_N == 8 && str_len > 1 && str_len <= 4) {
            ...
            if (char0_bytes == str_len) {
                ...
                if (char0_bytes == 2 && (v0 & 0xFC) == 0xC0) { // 0x110000xx 0x10xxxxxx
                    one_char =
                        static_cast<CharT>(((v0 & 3) << 6) + (static_cast<unsigned char>(value[1]) & 0x3F));
                    return one_char;
                }
...
template <template <typename...> class Tuple, typename... Ts> class tuple_caster {
  ...
  protected:
    ...
    template <size_t... Is> bool load_impl(const sequence& seq, bool convert, index_sequence<Is...>) {
...
struct copyable_holder_caster<
  ...
  public:
    ...
    bool load(handle src, bool convert) {
        if (base::template load_impl<copyable_holder_caster<type, std::shared_ptr<type>>>(src, convert)) {
    ...
    static handle cast(const std::shared_ptr<type>& src, return_value_policy policy, handle parent) {
        ...
        if (st.second->holder_enum_v == detail::holder_enum_t::smart_holder) {
            return smart_holder_type_caster_support::smart_holder_from_shared_ptr(src, policy, parent, st);
        }
        return type_caster_base<type>::cast_holder(ptr, &src);
    }
...
template <typename T> std::weak_ptr<T> potentially_slicing_weak_ptr(handle obj) {
...
template <typename type, typename holder_type, typename SFINAE = void> struct move_only_holder_caster {
...
struct move_only_holder_caster<
  ...
  public:
    ...
    static handle cast(std::unique_ptr<type, deleter>&& src, return_value_policy policy, handle parent) {
        ...
        if (st.second->holder_enum_v == detail::holder_enum_t::smart_holder) {
            return smart_holder_type_caster_support::smart_holder_from_unique_ptr(std::move(src), policy,
                                                                                  parent, st);
        }
        return type_caster_generic::cast(st.first, return_value_policy::take_ownership, {}, st.second,
                                         nullptr, nullptr, std::addressof(src));
    }

    static handle cast(const std::unique_ptr<type, deleter>& src, return_value_policy policy, handle parent) {
        if (!src) {
            return none().release();
        }
    ...
    bool load(handle src, bool convert) {
        if (base::template load_impl<move_only_holder_caster<type, std::unique_ptr<type, deleter>>>(
                src, convert)) {
            sh_load_helper.maybe_set_python_instance_is_alias(src);
            return true;
        }
    ...
    using cast_op_type = conditional_t<
        std::is_same<typename std::remove_volatile<T_>::type, const std::unique_ptr<type, deleter>&>::value ||
        ...
        if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
            ...
            unique_ptr_storage = std::shared_ptr<std::unique_ptr<type, deleter>>(
                new std::unique_ptr<type, deleter>{
                    sh_load_helper.template load_as_const_unique_ptr<deleter>(shared_ptr_storage.get())},
...
using type_caster_holder =
    conditional_t<is_copy_constructible<holder_type>::value, copyable_holder_caster<type, holder_type>,
                  move_only_holder_caster<type, holder_type>>;

template <bool Value = false> struct always_construct_holder_value { static constexpr bool value = Value; };
...
template <typename base> struct is_holder_type<base, smart_holder> : std::true_type {};
...
template <> struct handle_type_name<object> { static constexpr auto name = const_name("object"); };
template <> struct handle_type_name<list> { static constexpr auto name = const_name("list"); };
template <> struct handle_type_name<dict> { static constexpr auto name = const_name("dict"); };
template <> struct handle_type_name<anyset> { static constexpr auto name = const_name("set | frozenset"); };
template <> struct handle_type_name<set> { static constexpr auto name = const_name("set"); };
template <> struct handle_type_name<frozenset> { static constexpr auto name = const_name("frozenset"); };
template <> struct handle_type_name<str> { static constexpr auto name = const_name("str"); };
template <> struct handle_type_name<tuple> { static constexpr auto name = const_name("tuple"); };
...
template <> struct handle_type_name<handle> { static constexpr auto name = handle_type_name<object>::name; };
...
template <> struct handle_type_name<bytearray> { static constexpr auto name = const_name("bytearray"); };
template <> struct handle_type_name<memoryview> { static constexpr auto name = const_name("memoryview"); };
template <> struct handle_type_name<slice> { static constexpr auto name = const_name("slice"); };
template <> struct handle_type_name<type> { static constexpr auto name = const_name("type"); };
template <> struct handle_type_name<capsule> {
    static constexpr auto name = const_name(PYBIND11_CAPSULE_TYPE_TYPE_HINT);
};
template <> struct handle_type_name<ellipsis> { static constexpr auto name = const_name("ellipsis"); };
...
template <typename T> struct handle_type_name<Args<T>> {
...
template <typename T> struct handle_type_name<KWArgs<T>> {
...
template <typename T>
struct move_always<
    T, enable_if_t<all_of<move_is_plain_type<T>, negation<is_copy_constructible<T>>, is_move_constructible<T>,
                          std::is_same<decltype(std::declval<make_caster<T>>().operator T&()), T&>>::value>>
    : std::true_type {};
template <typename T, typename SFINAE = void> struct move_if_unreferenced : std::false_type {};
template <typename T>
struct move_if_unreferenced<
    T, enable_if_t<all_of<move_is_plain_type<T>, negation<move_always<T>>, is_move_constructible<T>,
                          std::is_same<decltype(std::declval<make_caster<T>>().operator T&()), T&>>::value>>
    : std::true_type {};
...
struct return_value_policy_override<
    Return, detail::enable_if_t<std::is_base_of<type_caster_generic, make_caster<Return>>::value, void>> {
...
#else
        throw cast_error("Unable to cast Python instance of type " +
                         str(type::handle_of(handle)).cast<std::string>() + " to C++ type '" + type_id<T>() +
                         "'");
#endif
    }
    return conv;
...
T cast(const handle& handle) {
    ...
    if (is_enum_cast && cast_is_temporary_value_reference<T>::value) {
        if (detail::global_internals_native_enum_type_map_contains(std::type_index(typeid(intrinsic_t<T>)))) {
...
template <typename T> detail::enable_if_t<!detail::move_never<T>::value, T> move(object&& obj) {
    if (obj.ref_count() > 1) {
#if !defined(PYBIND11_DETAILED_ERROR_MESSAGES)
        throw cast_error("Unable to cast Python " + str(type::handle_of(obj)).cast<std::string>() +
                         " instance to C++ rvalue: instance has multiple references"
                         " (#define PYBIND11_DETAILED_ERROR_MESSAGES or compile in debug mode for details)");
#else
        throw cast_error("Unable to move from Python " + str(type::handle_of(obj)).cast<std::string>() +
...
template <typename T> detail::enable_if_t<detail::is_pyobject<T>::value, T> cast(object&& object) {
...
template <typename T> std::string generate_type_signature();
...
template <typename T>
enable_if_t<
    cast_is_temporary_value_reference<T>::value && !detail::is_same_ignoring_cvref<T, PyObject*>::value, T>
cast_safe(object&&) {
    pybind11_fail("Internal error: cast_safe fallback invoked");
}
template <typename T> enable_if_t<std::is_void<T>::value, void> cast_safe(object&&) {}
template <typename T>
...
template <return_value_policy policy = return_value_policy::automatic_reference> tuple make_tuple() {
    return tuple(0);
}
...
template <typename... Args> class argument_loader {
    ...
    template <typename Arg> using argument_is_args = is_same_or_base_of<args, intrinsic_t<Arg>>;
    template <typename Arg> using argument_is_kwargs = is_same_or_base_of<kwargs, intrinsic_t<Arg>>;
  ...
  public:
    ...
    enable_if_t<!std::is_void<Return>::value, Return> call(Func&& f) && {
        return std::move(*this).template call_impl<remove_cv_t<Return>>(std::forward<Func>(f), indices{},
                                                                        Guard{});
    }
