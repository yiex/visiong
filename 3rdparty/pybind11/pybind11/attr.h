...
struct function_record {
    function_record()
        : is_constructor(false), is_new_style_constructor(false), is_stateless(false), is_operator(false),
          is_method(false), is_setter(false), has_args(false), has_kwargs(false), prepend(false) {}
...
struct type_record {
    PYBIND11_NOINLINE type_record()
        : multiple_inheritance(false), dynamic_attr(false), buffer_protocol(false), module_local(false),
          is_final(false), release_gil_before_calling_cpp_dtor(false) {}
    ...
    PYBIND11_NOINLINE void add_base(const std::type_info& base, void* (*caster)(void*)) {
        ...
        if (!base_info) {
            ...
            pybind11_fail("generic_type: type \"" + std::string(name) + "\" referenced unknown base type \"" +
                          tname + "\"");
        }
...
template <> struct process_attribute<is_setter> : process_attribute_default<is_setter> {
...
template <> struct process_attribute<arg_v> : process_attribute_default<arg_v> {
    static void init(const arg_v& a, function_record* r) {
        if (r->is_method && r->args.empty()) {
            r->args.emplace_back("self", /*descr=*/nullptr, /*parent=*/handle(), /*convert=*/true,
                                 /*none=*/false);
        }
...
template <> struct process_attribute<multiple_inheritance> : process_attribute_default<multiple_inheritance> {
...
template <> struct process_attribute<buffer_protocol> : process_attribute_default<buffer_protocol> {
