...
template <typename EnumType> class native_enum : public detail::native_enum_data {
  public:
    ...
    native_enum(const object& parent_scope, const char* name, const char* native_type_name,
                const char* class_doc = "")
        : detail::native_enum_data(parent_scope, name, native_type_name, class_doc,
        ...
        if (detail::global_internals_native_enum_type_map_contains(enum_type_index)) {
            pybind11_fail("pybind11::native_enum<...>(\"" + enum_name_encoded + "\") is already registered!");
        }
        arm_finalize_check();
    }
