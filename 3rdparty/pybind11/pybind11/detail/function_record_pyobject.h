...
static PyMethodDef tp_methods_impl[] = {
    {"__reduce_ex__",
     ...
     reinterpret_cast<PyCFunction>(reinterpret_cast<void*>(reduce_ex_impl)), METH_VARARGS | METH_KEYWORDS,
     nullptr},
...
inline PyObject* reduce_ex_impl(PyObject* self, PyObject*, PyObject*) {
    ...
    if (rec == nullptr) {
        pybind11_fail("FATAL: function_record_PyTypeObject reduce_ex_impl(): cannot obtain cpp_func_rec.");
    }
    if (rec->name != nullptr && rec->name[0] != '\0' && rec->scope && PyModule_Check(rec->scope.ptr()) != 0) {
        ...
        if (scope_module) {
            ...
            auto reconstruct_args =
                make_tuple(str("__import__('importlib').import_module('") + scope_module + str("')"));
            return make_tuple(std::move(builtins_eval), std::move(reconstruct_args)).release().ptr();
        }
    }
