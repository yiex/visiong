...
// Default fallback.
static constexpr bool type_has_shared_from_this(...) {
    return false;
}
...
template <typename T> static constexpr bool type_has_shared_from_this(const void*) {
    return false;
}
...
struct guarded_delete {
    ...
    guarded_delete(std::function<void(void*)>&& del_fun, bool armed_flag)
        : del_fun{std::move(del_fun)}, del_ptr{nullptr}, use_del_fun{true}, armed_flag{armed_flag} {}
...
template <typename T> guarded_delete make_guarded_builtin_delete(bool armed_flag) {
    return guarded_delete(builtin_delete_if_destructible<T>, armed_flag);
}

template <typename T, typename D> struct custom_deleter {
    D deleter;
    explicit custom_deleter(D&& deleter) : deleter{std::forward<D>(deleter)} {}
    void operator()(void* raw_ptr) { deleter(static_cast<T*>(raw_ptr)); }
};

template <typename T, typename D> guarded_delete make_guarded_custom_deleter(D&& uqp_del, bool armed_flag) {
    return guarded_delete(std::function<void(void*)>(custom_deleter<T, D>(std::forward<D>(uqp_del))),
                          armed_flag);
}

template <typename T> inline bool is_std_default_delete(const std::type_info& rtti_deleter) {
...
struct smart_holder {
    ...
    template <typename T> static void ensure_pointee_is_destructible(const char* context) {
    ...
    void ensure_vptr_is_using_builtin_delete(const char* context) const {
        if (vptr_is_external_shared_ptr) {
            throw std::invalid_argument(std::string("Cannot disown external shared_ptr (") + context + ").");
        }
    ...
    template <typename T, typename D> void ensure_compatible_rtti_uqp_del(const char* context) const {
        ...
        if (!rtti_uqp_del) {
            ...
            ensure_vptr_is_using_builtin_delete(context);
                   ...
                   !(vptr_is_using_builtin_delete && is_std_default_delete<T>(*rtti_requested))) {
    ...
    template <typename T, typename D> std::unique_ptr<D> extract_deleter(const char* context) const {
    ...
    template <typename T> T* as_raw_ptr_unowned() const { return static_cast<T*>(vptr.get()); }
    ...
    static smart_holder from_unique_ptr(std::unique_ptr<T, D>&& unq_ptr, void* void_ptr = nullptr) {
        smart_holder hld;
    ...
    template <typename T> static smart_holder from_shared_ptr(const std::shared_ptr<T>& shd_ptr) {
        smart_holder hld;
    ...
    template <typename T> std::shared_ptr<T> as_shared_ptr() const {
