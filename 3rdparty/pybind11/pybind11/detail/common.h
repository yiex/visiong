...
#define Py_PACK_FULL_VERSION(X, Y, Z, LEVEL, SERIAL)                                                         \
    ((((X)&0xff) << 24) | (((Y)&0xff) << 16) | (((Z)&0xff) << 8) | (((LEVEL)&0xf) << 4) |                    \
     (((SERIAL)&0xf) << 0))
#endif
// Version as a single 4-byte hex number, e.g. 0x030C04B5 == 3.12.4b5.
#define PYBIND11_VERSION_HEX                                                                                 \
    Py_PACK_FULL_VERSION(PYBIND11_VERSION_MAJOR, PYBIND11_VERSION_MINOR, PYBIND11_VERSION_MICRO,             \
                         PYBIND11_VERSION_RELEASE_LEVEL, PYBIND11_VERSION_RELEASE_SERIAL)
...
#if defined(PYBIND11_CPP17) && ((defined(__has_include) && __has_include(<string_view>)) || defined(_MSC_VER))
...
#if !defined(PYBIND11_NO_ASSERT_GIL_HELD_INCREF_DECREF) && !defined(NDEBUG) && !defined(PYPY_VERSION) &&     \
...
#define PYBIND11_PLUGIN_DECL(name) extern "C" PYBIND11_MAYBE_UNUSED PYBIND11_EXPORT PyObject* PyInit_##name();
...
#define PYBIND11_MODULE_PYINIT(name, pre_init, ...)                                                          \
    ...
    PYBIND11_PLUGIN_IMPL(name) {                                                                             \
        ...
        pre_init;                                                                                            \
        PYBIND11_ENSURE_INTERNALS_READY                                                                      \
        static ::pybind11::detail::slots_array slots =                                                       \
            ::pybind11::detail::init_slots(&PYBIND11_CONCAT(pybind11_exec_, name), ##__VA_ARGS__);           \
...
#define PYBIND11_MODULE_EXEC(name, variable)                                                                 \
    ...
    void PYBIND11_CONCAT(pybind11_init_,                                                                     \
                         name)(::pybind11::module_ & variable) // NOLINT(bugprone-macro-parentheses)
...
template <typename T, typename U> using is_same_ignoring_cvref = std::is_same<detail::remove_cvref_t<T>, U>;
...
struct select_indices_impl<index_sequence<IPrev...>, I, B, Bs...>
    : select_indices_impl<conditional_t<B, index_sequence<IPrev..., I>, index_sequence<IPrev...>>, I + 1,
                          Bs...> {};
...
template <template <typename> class Predicate, typename Default, typename... Ts> struct exactly_one {
...
template <template <typename...> class Class, typename T> struct is_instantiation : std::false_type {};
...
using function_signature_t =
    conditional_t<std::is_function<F>::value, F,
...
template <typename T>
using is_lambda =
    satisfies_none_of<remove_reference_t<T>, std::is_function, std::is_pointer, std::is_member_pointer>;
...
template <typename T> struct is_fmt_numeric<T, enable_if_t<std::is_arithmetic<T>::value>> {
    ...
    static constexpr int index =
        std::is_same<T, bool>::value
            ? 0
            : 1 + (std::is_integral<T>::value ? detail::log2(sizeof(T)) * 2 + std::is_unsigned<T>::value
...
template <typename... Args> static constexpr detail::overload_cast_impl<Args...> overload_cast{};
#endif
...
template <typename T> class any_container {
  ...
  public:
    ...
    template <typename Container, typename = enable_if_t<std::is_convertible<
