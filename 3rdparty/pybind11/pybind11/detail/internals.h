...
static_assert(sizeof(instance_map_shard) % 64 == 0, "instance_map_shard size is not a multiple of 64 bytes");
...
struct internals {
    ...
    internals() {
        ...
        if (!PYBIND11_TLS_KEY_CREATE(tstate)) {
            pybind11_fail("internals constructor: could not successfully initialize the tstate TSS key!");
        }
...
template <typename InternalsType> inline std::unique_ptr<InternalsType>*& get_internals_pp() {
...
get_internals_pp_from_capsule_in_state_dict(dict& state_dict, char const* state_dict_key) {
    auto internals_obj = reinterpret_steal<object>(dict_getitemstringref(state_dict.ptr(), state_dict_key));
    if (internals_obj) {
...
PYBIND11_NOINLINE internals& get_internals() {
    ...
    internals_pp = get_internals_pp_from_capsule_in_state_dict<internals>(state_dict, PYBIND11_INTERNALS_ID);
    if (!internals_pp) {
    ...
    if (*internals_pp) {
        ...
        if ((*internals_pp)->registered_exception_translators.empty() ||
            (*internals_pp)->registered_exception_translators.front() != &translate_local_exception) {
...
inline char const* get_local_internals_id() {
    ...
    static const std::string this_module_idstr =
        PYBIND11_MODULE_LOCAL_ID + std::to_string(reinterpret_cast<uintptr_t>(&this_module_idstr));
...
inline local_internals& get_local_internals() {
    ...
    dict state_dict = get_python_state_dict();
    local_internals_pp =
        get_internals_pp_from_capsule_in_state_dict<local_internals>(state_dict, get_local_internals_id());
    if (!local_internals_pp) {
        local_internals_pp = new std::unique_ptr<local_internals>;
        state_dict[get_local_internals_id()] = capsule(reinterpret_cast<void*>(local_internals_pp));
    }
    if (!*local_internals_pp) {
...
template <typename F> inline auto with_internals(const F& cb) -> decltype(cb(get_internals())) {
...
inline auto with_exception_translators(const F& cb)
    ...
    return cb(internals.registered_exception_translators, local_internals.registered_exception_translators);
}
...
template <typename F>
inline auto with_instance_map(const void* ptr, const F& cb) -> decltype(cb(std::declval<instance_map&>())) {
    auto& internals = get_internals();
