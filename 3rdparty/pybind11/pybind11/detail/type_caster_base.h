...
class loader_life_support {
  private:
    ...
    static PYBIND11_TLS_KEY_REF get_stack_tls_key() { return get_internals().loader_life_support_tls_key; }
...
inline void all_type_info_add_base_most_derived_first(std::vector<type_info*>& bases, type_info* addl_base) {
...
PYBIND11_NOINLINE detail::type_info* get_type_info(const std::type_index& tp, bool throw_if_missing = false) {
    ...
    if (throw_if_missing) {
        ...
        pybind11_fail("pybind11::detail::get_type_info: unable to find type info for \"" + std::move(tname) +
                      '"');
    }
    return nullptr;
}
...
struct values_and_holders {
  ...
  public:
    ...
    explicit values_and_holders(PyObject* obj) : inst{nullptr}, tinfo(all_type_info(Py_TYPE(obj))) {
        if (!tinfo.empty()) {
    ...
    struct iterator {
      private:
        ...
        iterator(instance* inst, const type_vec* tinfo) : inst{inst}, types{tinfo} {
            if (inst != nullptr) {
                assert(!types->empty());
                curr = value_and_holder(inst /* instance */, (*types)[0] /* type info */,
...
handle smart_holder_from_unique_ptr(std::unique_ptr<T, D>&& src, return_value_policy policy, handle parent,
...
template <typename T, typename D>
handle smart_holder_from_unique_ptr(std::unique_ptr<T const, D>&& src, return_value_policy policy,
                                    handle parent, const std::pair<const void*, const type_info*>& st) {
    return smart_holder_from_unique_ptr(std::unique_ptr<T, D>(const_cast<T*>(src.release()),
                                                              std::move(src.get_deleter())), // Const2Mutbl
                                        policy, parent, st);
}

template <typename T>
handle smart_holder_from_shared_ptr(const std::shared_ptr<T>& src, return_value_policy policy, handle parent,
                                    const std::pair<const void*, const type_info*>& st) {
    switch (policy) {
...
template <typename T>
handle smart_holder_from_shared_ptr(const std::shared_ptr<T const>& src, return_value_policy policy,
                                    handle parent, const std::pair<const void*, const type_info*>& st) {
...
struct shared_ptr_parent_life_support {
    PyObject* parent;
    explicit shared_ptr_parent_life_support(PyObject* parent) : parent{parent} { Py_INCREF(parent); }
    // NOLINTNEXTLINE(readability-make-member-function-const)
    void operator()(void*) {
...
template <typename T> struct load_helper : value_and_holder_helper {
    ...
    std::shared_ptr<T> load_as_shared_ptr(void* void_raw_ptr, handle responsible_parent = nullptr,
        ...
        if (python_instance_is_alias && !force_potentially_slicing_shared_ptr) {
            ...
            if (vptr_gd_ptr != nullptr) {
                ...
                std::shared_ptr<T> to_be_released(type_raw_ptr,
            ...
            if (sptsls_ptr != nullptr || !memory::type_has_shared_from_this(type_raw_ptr)) {
                return std::shared_ptr<T>(type_raw_ptr,
                                          shared_ptr_trampoline_self_life_support(loaded_v_h.inst));
            }
            ...
            pybind11_fail("smart_holder_type_casters: load_as_shared_ptr failure: internal inconsistency.");
        }
    ...
    template <typename D>
    std::unique_ptr<T, D> load_as_unique_ptr(void* raw_void_ptr, const char* context = "load_as_unique_ptr") {
        if (!have_holder()) {
    ...
    std::unique_ptr<T, D> load_as_const_unique_ptr(T* raw_type_ptr,
        ...
        return unique_with_deleter<T, D>(raw_type_ptr,
                                         std::move(holder().template extract_deleter<T, D>(context)));
    }
};
...
inline object cpp_conduit_method(handle self, const bytes& pybind11_platform_abi_id,
                                 const capsule& cpp_type_info_capsule, const bytes& pointer_kind) {
...
template <typename Container, typename SFINAE = void> struct container_mapped_type_traits {
...
// Implemented by specializations below.
template <typename Container, typename SFINAE = void> struct container_value_type_traits : std::false_type {
...
struct container_value_type_traits<
    Container,
    typename std::enable_if<negation<std::is_same<typename Container::value_type, Container>>::value>::type> {
    static constexpr bool has_value_type = true;
    static constexpr bool has_recursive_value_type = false;
};
 ...
 */
template <typename T, bool is_this_a_map> struct impl_type_to_check_recursively {
    /*
 ...
 */
template <typename Container, typename SFINAE = void> struct impl_recursive_container_traits {
...
template <typename type> class type_caster_base : public type_caster_generic {
  ...
  public:
    ...
    static handle cast(const itype* src, return_value_policy policy, handle parent) {
        ...
        return type_caster_generic::cast(st.first, policy, parent, st.second, make_copy_constructor(src),
