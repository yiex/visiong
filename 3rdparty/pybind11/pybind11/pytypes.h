...
template <typename Derived> class object_api : public pyobject_tag {
  ...
  public:
    ...
    \endrst */
    template <typename T> obj_attr_accessor attr_with_type_hint(handle key) const;
    /// See above (the only difference is that the key is provided as a string literal)
    template <typename T> str_attr_accessor attr_with_type_hint(const char* key) const;
...
template <typename T>
using is_pyobj_ptr_or_nullptr_t = detail::any_of<std::is_same<T, PyObject*>, std::is_same<T, PyObject* const>,
  ...
  private:
    ...
    void throw_gilstate_error(const std::string& function_name) const {
        ...
        if (Py_TYPE(m_ptr)->tp_name != nullptr) {
            fprintf(stderr, " The failing %s call was triggered on a %s object.", function_name.c_str(),
...
struct error_fetch_and_normalize {
    ...
    explicit error_fetch_and_normalize(const char* called) {
        ...
        if (exc_type_name_norm != m_lazy_error_string) {
            std::string msg = std::string(called) + ": MISMATCH of original and normalized "
...
template <typename T, detail::enable_if_t<!std::is_base_of<object, T>::value, int> = 0>
bool isinstance(handle obj) {
    return detail::isinstance_native_enum<T>(obj, typeid(T)) || detail::isinstance_generic(obj, typeid(T));
}
...
template <typename T> using is_positional = satisfies_none_of<T, is_keyword, is_s_unpacking, is_ds_unpacking>;
template <typename T> using is_keyword_or_ds = satisfies_any_of<T, is_keyword, is_ds_unpacking>;

// Call argument collector forward declarations
template <return_value_policy policy = return_value_policy::automatic_reference> class simple_collector;
template <return_value_policy policy = return_value_policy::automatic_reference> class unpacking_collector;
...
class slice : public object {
  public:
    ...
    bool compute(size_t length, size_t* start, size_t* stop, size_t* step, size_t* slicelength) const {
        return PySlice_GetIndicesEx((PYBIND11_SLICE_OBJECT*)m_ptr, (ssize_t)length, (ssize_t*)start,
                                    (ssize_t*)stop, (ssize_t*)step, (ssize_t*)slicelength) == 0;
    }
    bool compute(ssize_t length, ssize_t* start, ssize_t* stop, ssize_t* step, ssize_t* slicelength) const {
        return PySlice_GetIndicesEx((PYBIND11_SLICE_OBJECT*)m_ptr, length, start, stop, step, slicelength) ==
               0;
    }
...
class capsule : public object {
  public:
    ...
    explicit capsule(const void* value, const char* name = nullptr, PyCapsule_Destructor destructor = nullptr)
  ...
  private:
    ...
    void initialize_with_void_ptr_destructor(const void* value, const char* name, void (*destructor)(void*)) {
...
class list : public object {
  public:
    ...
    void insert(const IdxType& index, ValType&& val) /* py-non-const */ {
        if (PyList_Insert(m_ptr, ssize_t_cast(index),
...
template <typename T> class Args : public args { using args::args; };

template <typename T> class KWArgs : public kwargs { using kwargs::kwargs; };
...
class memoryview : public object {
  public:
    ...
    static memoryview from_buffer(T* ptr, detail::any_container<ssize_t> shape,
                                  detail::any_container<ssize_t> strides, bool readonly = false) {
        return memoryview::from_buffer(reinterpret_cast<void*>(ptr), sizeof(T), format_descriptor<T>::value,
    ...
    static memoryview from_buffer(const T* ptr, detail::any_container<ssize_t> shape,
                                  detail::any_container<ssize_t> strides) {
        return memoryview::from_buffer(const_cast<T*>(ptr), std::move(shape), std::move(strides), true);
    }
...
template <typename D> item_accessor object_api<D>::operator[](object&& key) const {
...
template <typename D> obj_attr_accessor object_api<D>::attr(object&& key) const {
    return {derived(), std::move(key)};
}
...
template <typename D> object object_api<D>::annotations() const {
