...
inline bool object_is_convertible_to_std_vector(const handle& src) {
    ...
    return (PyGen_Check(src.ptr()) != 0) || (PyAnySet_Check(src.ptr()) != 0) ||
           object_is_instance_with_one_of_tp_names(src.ptr(),
...
template <typename T, typename U> constexpr forwarded_type<T, U> forward_like(U&& u) {
    return std::forward<detail::forwarded_type<T, U>>(std::forward<U>(u));
}

// Checks if a container has a STL style reserve method.
// This will only return true for a `reserve()` with a `void` return.
template <typename C> using has_reserve_method = std::is_same<decltype(std::declval<C>().reserve(0)), void>;
...
template <typename Type, typename Key> struct set_caster {
  ...
  public:
    ...
    template <typename T> static handle cast(T&& src, return_value_policy policy, handle parent) {
        ...
        for (auto&& value : src) {
            auto value_ =
                reinterpret_steal<object>(key_conv::cast(detail::forward_like<T>(value), policy, parent));
    ...
    PYBIND11_TYPE_CASTER(type, io_name("collections.abc.Set", "set") + const_name("[") + key_conv::name +
                                   const_name("]"));
};
...
template <typename Type, typename Key, typename Value> struct map_caster {
  ...
  public:
    ...
    PYBIND11_TYPE_CASTER(Type, io_name("collections.abc.Mapping", "dict") + const_name("[") + key_conv::name +
                                   const_name(", ") + value_conv::name + const_name("]"));
};
...
template <typename Type, typename Value> struct list_caster {
  ...
  public:
    template <typename T> static handle cast(T&& src, return_value_policy policy, handle parent) {
        ...
        for (auto&& value : src) {
            auto value_ =
                reinterpret_steal<object>(value_conv::cast(detail::forward_like<T>(value), policy, parent));
            if (!value_) {
                return handle();
            }
    ...
    PYBIND11_TYPE_CASTER(Type, io_name("collections.abc.Sequence", "list") + const_name("[") +
...
template <typename ArrayType, size_t N, typename V> ArrayType vector_to_array(V&& v) {
...
template <typename ArrayType, typename Value, bool Resizable, size_t Size = 0> struct array_caster {
  ...
  private:
    ...
    template <bool R = Resizable, enable_if_t<R, int> = 0> bool convert_elements(handle seq, bool convert) {
  ...
  public:
    ...
    template <typename T_> using cast_op_type = movable_cast_op_type<T_>;

    static constexpr auto name =
        const_name<Resizable>(const_name(""), const_name("typing.Annotated[")) +
        io_name("collections.abc.Sequence", "list") + const_name("[") + value_conv::name + const_name("]") +
        const_name<Resizable>(const_name(""),
                              const_name(", \"FixedSize(") + const_name<Size>() + const_name(")\"]"));
};
...
#if defined(PYBIND11_HAS_OPTIONAL)
template <typename T> struct type_caster<std::optional<T>> : public optional_caster<std::optional<T>> {};
...
template <template <typename...> class V, typename... Ts> struct variant_caster<V<Ts...>> {
    ...
    template <typename Variant> static handle cast(Variant&& src, return_value_policy policy, handle parent) {
...
#if defined(PYBIND11_HAS_VARIANT)
template <typename... Ts> struct type_caster<std::variant<Ts...>> : variant_caster<std::variant<Ts...>> {};
